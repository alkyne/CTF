<?php session_start(); include("5f0c2baaa2c0426eed9a958e3fe0ff94.php"); // nothing here ?>
<?php
$page = $_GET['page'];
if($page == "login"){
?>
...
<?php }
  ...
  if(($_POST['uid']) && ($_POST['upw'])){
    ...
    if(strlen($_POST['uid']) > 32) exit("id too long");
  ...
  if(!$_SESSION['uid']) exit("<script>alert('login first');location.href='./?page=login';</script>");
  if(!$_POST['letter']) exit("<form method=post><textarea name=letter placeholder=\"letter to krystal\"></textarea><br><input type=submit></form>");
  else{
    $letter = addslashes($_POST['letter']);
    mysql_query("insert into letter values('{$_SESSION[uid]}','{$letter}')");
    if(mysql_error()) exit("something wrong");
    echo "<script>alert('send ok');location.href='./';</script>";
  }
}
